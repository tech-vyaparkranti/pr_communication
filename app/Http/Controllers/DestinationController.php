...
class DestinationController extends Controller
{
    ...
    public function applyDestination(DestinationsModel $id)
    {
        if ($id->{DestinationsModel::STATUS} == 1) {
            $data = (new HomePageController())->getElement();
            $data["Destination_details"] = $id;
            return view("HomePage.Destination.DestinationDetails", $data);
        } else {
            return redirect()->back()->with("error", "Destination details not found.");
        }
    }

    public function demoDestination(DestinationsModel $id)
    {
        if ($id->{DestinationsModel::STATUS} == 1) {
            $data = (new HomePageController())->getElement();
            $data["Destination_details"] = $id;
            $data["Destination_lessons"] = $this->getDestinationMaterial($id->id);
            return view("HomePage.Destination.DestinationLessons", $data);
        } else {
            return redirect()->back()->with("error", "Destination details not found.");
        }
    }
